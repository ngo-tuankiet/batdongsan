      ...
      <div style="padding: 26px;">
        ...
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <a :href="`tel:${(property.agent?.phone || '0912131329').replace(/[^0-9]/g, '')}`" class="btn btn-call" style="flex: 1;">
            <i class="fa-solid fa-phone"></i> Gọi Tư Vấn: {{ property.agent?.phoneDisplay || property.agent?.phone || '0912.131.329' }}
          </a>
        ...
        <NuxtLink :to="`/bat-dong-san/${property.id}`" class="btn btn-outline-gold" style="width: 100%; justify-content: center; margin-top: 10px; font-size: 0.85rem;" @click="$emit('close')">
          <i class="fa-solid fa-arrow-up-right-from-square"></i> Mở Toàn Màn Hình & Thư Viện Ảnh (URL Riêng)
        </NuxtLink>
      </div>
    </div>
  </div>
